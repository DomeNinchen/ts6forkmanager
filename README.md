...
- Custom avatar (PNG/JPEG/GIF/WebP, up to 2MB) and a templated `client_description`, set from the Create/Edit Music Bot dialog and applied live. The description updates whenever something actually changes (song start/change, queue size, editing the template) - plus every ~30s while playing, but only if the template contains a time placeholder like `{remaining}`, so it isn't polled needlessly otherwise. Shows a "-"/"0" idle form when nothing is playing instead of going blank. Supports:
